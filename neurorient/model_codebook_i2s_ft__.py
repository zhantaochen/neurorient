...
class Slice2RotMat_CodeBook(nn.Module):
    ...
    def forward(self, image):
        ...
        probs_inp = torch.nn.functional.softmax(logits_pred, dim=-1).float()
        loss_neg_entropy = torch.sum(probs_inp * torch.log(probs_inp + 1e-8), dim=-1).mean()

        R_inp = compute_weighted_average_so3(self.i2s.output_rotmats, probs_inp)
        ...
        R_base = torch.einsum('bn, nij->bij', probs_oh, self.ref_rotations)

        image_ref = self.ref_images[indices]
        probs_ref = self.i2s.compute_logits(image_ref.unsqueeze(1)).softmax(dim=-1)
        R_ref = compute_weighted_average_so3(self.i2s.output_rotmats, probs_ref)

        R_out = torch.einsum('bij, bjk, bkl -> bil', R_base, R_ref.transpose(2,1), R_inp)
