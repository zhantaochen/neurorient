...
from .external.rsh import rsh_cart_6
...
class IntensityNet(nn.Module):
    ...
    #     scales = torch.exp(- F.relu(self.alpha) * x_norm)
    #     return 1/2 * (self.net_mag(x)+self.net_mag(-x)) * scales, self.f_q(x_norm) * scales

    def forward(self, x):
        x_norm = x.norm(dim=-1, keepdim=True)
        x_normed = x / (x_norm + 1e-6)
        scales = torch.exp(- F.relu(self.alpha) * x_norm)
        if self.f_hkl is None:
            return 1/2 * (self.f_tp(x_normed)+self.f_tp(-x_normed)) * self.f_q(x_norm) * scales
        else:
            return 1/2 * (self.f_tp(x_normed)+self.f_tp(-x_normed)) * self.f_q(x_norm) * scales, self.f_hkl(x) * scales
...
class NeurOrient(nn.Module):
    ...
    def predict_intensity(self, grid_position_reciprocal):
        ...
        f_q = self.volume_predictor(grid_position_reciprocal)
        q_HKL = grid_position_reciprocal.norm(dim=-1, keepdim=True)
        Ylm = rsh_cart_6(grid_position_reciprocal / (q_HKL + 1e-6))[...,-13:]
        
        if isinstance(f_q, torch.Tensor):
            slices_D6 = torch.einsum('q, qj, j -> q', f_q.squeeze(-1), Ylm, self.d).pow(2)
            intensity = slices_D6
        elif isinstance(f_q, (tuple, list)):
            slices_D6 = torch.einsum('q, qj, j -> q', f_q[0].squeeze(-1), Ylm, self.d).pow(2)
            intensity = slices_D6 + f_q[1].squeeze(-1).pow(2)
        # print(slices_D6.max(), d_I.max(), intensity.max())
