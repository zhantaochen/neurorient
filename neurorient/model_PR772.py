...
class Slice2RotMat_I2S(nn.Module):
    def __init__(self, input_size, mode='sampling', rec_level=2, lmax=3, size=18, pretrained=False):
        super().__init__()
...
class NeurOrientLightning(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.model.fluctuation_predictor is not None:
            ...
            slices_scale_factor = slices_scale_factor.repeat_interleave(slices_pred.shape[0]//slices_scale_factor.shape[0], dim=0)
        ...
        # loss = model.loss_func((slices_scale_factor * slices_pred)[general_mask.bool()].cpu(), slices_target[general_mask.bool()].cpu())
        loss_recon = self.loss_func(slices_pred[general_mask.repeat_interleave(slices_pred.shape[0]//general_mask.shape[0], dim=0).bool()].cpu(), 
                                    slices_target[general_mask.bool()].cpu().repeat_interleave(slices_pred.shape[0]//slices_target.shape[0], dim=0))
        loss = loss_recon 
        self.log("train_loss", loss.item(), prog_bar=True, sync_dist=True)
        self.log("train_loss_recon", loss_recon.item(), sync_dist=True)
        
        # w = self.get_rotation_diversity_loss_weight()
        # loss_rodiv = self.rotation_diversity_loss(orientations.reshape(-1,3,3))
        # loss = loss_recon + w * loss_rodiv
        # self.log("train_loss", loss.item(), prog_bar=True, sync_dist=True)
        # self.log("train_loss_recon", loss_recon.item(), sync_dist=True)
        # self.log("train_loss_rodiv", loss_rodiv.item())
        # self.log("rodiv_weight", w)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # self.log("val_loss", loss.item(), prog_bar=True, sync_dist=True)
        # self.log("val_loss_recon", loss_recon.item(), sync_dist=True)
        # self.log("val_loss_rodiv", loss_rodiv.item())
