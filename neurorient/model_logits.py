...
class NeurOrient(nn.Module):
    ...
    def image_to_orientation(self, x, *args, **kwargs):
        rotmats = self.orientation_predictor(x, *args, **kwargs)
        return rotmats
    ...
    def estimate(self, x, input_mask=None, general_mask=None, log_transform=False):
        ...
        # it predicts log(1 + I)
        slices_pred = self.predict_slice(HKL).view((-1, 1,) + (self.image_dimension,)*2)
        if self.fluctuation_predictor is not None:
            slices_scale_factor = self.fluctuation_predictor(slices_input).unsqueeze(-1).unsqueeze(-1)
            if log_transform:
                slices_target  = general_mask * torch.log(slices_true * self.loss_scale_factor + 1.)
                slices_pred    = torch.log((torch.exp(slices_pred) - 1) * slices_scale_factor + 1)
            else:
                slices_target  = general_mask * slices_true * self.loss_scale_factor
...
class NeurOrientLightning(L.LightningModule):
    ...
    def __init__(self, 
        ...
        self.get_ref_images_every_n_steps = 25
    ...
    def _step(self, batch, batch_idx):
        ...
        if self.log_transform:
            ...
            slice_disp = slices_pred[:num_figs] / self.model.loss_scale_factordisplay_images_in_parallel(slice_disp, slices_true[:num_figs], save_to=f'{self.fig_path}/version_{self.logger.version}_train.png')
        ...
        return {
            ...
            'slices_true': slices_true[:num_figs],
            'slice_disp_input': slice_disp_input,
            'slice_disp_output': slice_disp_output,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step == 0 or self.global_step % self.get_ref_images_every_n_steps == 0:
            self.model.orientation_predictor.get_ref_images(
                self.model.volume_predictor, self.model.pixel_position_reciprocal, self.model.image_dimension, self.model.over_sampling)
        ...
        if self.global_step % 10 == 0:
            ...
            display_images_in_parallel(slice_disp, slices_true[:num_figs], 
                                       cmap='magma',
                                       save_to=f'{self.fig_path}/version_{self.logger.version}_train.png')
            if isinstance(input_mask, torch.Tensor):
                slice_disp_input = (torch.exp(slices_input[:num_figs]) - 1) / self.model.loss_scale_factor
                display_images_in_parallel(input_mask[:num_figs], slice_disp_input, 
                                           titles = ('Input Masks', 'Input Slices'),
                                           cmap='magma',
                                           save_to=f'{self.fig_path}/version_{self.logger.version}_{task_prefix}_in.png')
            if isinstance(general_mask, torch.Tensor):
                if self.log_transform:
                    slice_disp_output = (torch.exp(slices_target[:num_figs]) - 1) / self.model.loss_scale_factor
                else:
                ...
                display_images_in_parallel(general_mask[:num_figs], slice_disp_output, 
                                           ...
                                           cmap='magma',
                                           save_to=f'{self.fig_path}/version_{self.logger.version}_{task_prefix}_out.png')
            
            reciprocal_volume = self.predict_reciprocal_volume()
            display_volumes(reciprocal_volume, closefig=True, cmap='magma',
                            # vmax=max(1e-3 * reciprocal_volume.max(), reciprocal_volume.min()),
    ...
    def predict_reciprocal_volume_lin_scale(self, zoom=1.0):
        grid_reciprocal = np.pi * self.model.grid_position_reciprocal / self.model.grid_position_reciprocal.max()
        if zoom != 1.0:
            grid_reciprocal = scipy.ndimage.zoom(grid_reciprocal.detach().cpu().numpy(), (zoom,zoom,zoom,1), order=1)
            grid_reciprocal = torch.from_numpy(grid_reciprocal).to(self.device)
        ...
        volume = (np.exp(volume.clip(0.0)) - 1.) / self.model.loss_scale_factor
        # volume = np.exp(volume) / self.model.loss_scale_factor
        return volume