...
class ClassPhaser():
    ...
    def phase(self, algorithms, qlabel=None):
        ...
        x, y, z = np.indices(self.current.shape)
        cen = self.current.shape[-1] // 2
        shift = int(cen - (x*self.support).sum() / self.support.sum()), int(cen - (y*self.support).sum() / self.support.sum()), \
            int(cen - (z*self.support).sum() / self.support.sum())
        self.current = np.roll(self.current, shift, axis=(0,1,2))
        self.support = np.roll(self.support, shift, axis=(0,1,2))
        shift = int(cen - (x*self.support).sum() / self.support.sum()), int(cen - (y*self.support).sum() / self.support.sum()), \
            int(cen - (z*self.support).sum() / self.support.sum())
        self.current = np.roll(self.current, shift, axis=(0,1,2))
        self.support = np.roll(self.support, shift, axis=(0,1,2))
