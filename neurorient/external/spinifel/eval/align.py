...
def rotate_volume(vol, quat):
    ...
    """
    # print("using new version of rotate_volume")
    vol = xp.array(vol)
    quat = xp.array(quat)

    M = vol.shape[0]
    lincoords = xp.arange(M)
    coords = xp.meshgrid(lincoords, lincoords, lincoords, indexing='ij')

    xyz = xp.vstack([coords[0].reshape(-1) - M/2,
                     coords[1].reshape(-1) - M/2,
                     coords[2].reshape(-1) - M/2])

    R = quaternion2rot3d(quat)
    inv_R = xp.moveaxis(R, (1,2), (2,1))
    transformed_xyz = xp.dot(inv_R, xyz) + M/2

    new_xyz = xp.array([transformed_xyz[:, 0, :].flatten(),
