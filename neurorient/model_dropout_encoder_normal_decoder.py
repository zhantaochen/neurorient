...
class Slice2RotMat_MH(nn.Module):
    def __init__(self, size=18, pretrained=False):
        super().__init__()
        ...
        self.resnet.conv1.weight.data = conv1_weight
        # Output 6D rotation matrix
        self.resnet.fc = nn.Linear(self.resnet.fc.in_features, 6 * self.N_heads)
    
    def forward(self, img):
        if img.ndim == 3:
            img = img.unsqueeze(1)
        embed = self.resnet(img).reshape(-1, self.N_heads, 6)
        head_indices = torch.randint(0, self.N_heads, (embed.shape[0],)).to(img.device)  # Shape (a,)
        rotmat = rotation_6d_to_matrix(embed[torch.arange(embed.shape[0]).to(img.device), head_indices, :])
        # print(embed.shape, head_indices.shape, rotmat.shape)
        return {'rotations': rotmat}
...
class NeurOrient(nn.Module):
    def __init__(self, 
        ...
        self.orientation_predictor = Slice2RotMat_MH(**config_slice2rotmat)
